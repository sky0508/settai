...
1. **確定判定は薄く、加減点で"らしさ"を出す** — 機械で確実に取れるデータ（予算・立地・ジャンル・個室・競合ビール）は少ない。これだけで骨格を作り、接待の"効き"は加減点レイヤーで表現する。
...
- 🔴 **`settai_records` 蓄積** = 接待で**本当に効く加減点（黒子力・予約堅牢性・粋・重複回避）はここが無いと成立しない**。だから記録を「あとで足す」ではなく**設計の中心**に置く。使うほど賢くなる＝模倣困難な差別化。
