...
会食の店選び支援ツール。相手企業ごとの制約（例: キリン→競合ビール専売の店はNG）と、新人のセンス不足による失礼を、NG理由付きのおすすめランキングで防ぐ。会食記録を評価・シチュエーション付きで蓄積し社内ナレッジ基盤へ育てる。
...
- [x] `docs/research/settai-venue-selection-research.md` — 会食の店選び解像度上げリサーチ（2026-07-08, 9軸A〜Iへマッピング済み・ヒアリング設問への反映提案付き）
...
- v1 地理: 東京駅・京橋周辺
- 差別化: 会食記録＋評価＋シチュエーションによる社内ナレッジ共有
...
| C. 会食オペ適性      | 良い店は黒子。会計を席で見せない/仲居が空気を読む/「いつもの」が効く            | 相手の前で伝票 / 料理ペースが乱れる       | データ化不可→**会食実績タグ＋社内記録が唯一の源**           |           |
...
- "使うほど賢くなる"が本当に効くのは C（会食オペ適性）。一度使わないと分からない＝社内記録が効く一点突破の根拠。
...
- **impl repo を clone → 実地インスペクション**（public repo・`/Users/sorasasaki/work-os/02_projects/settai-navi/impl`）。判明した重要事実:
  ...
  - `map/page.tsx` は **「会食記録が1件以上ある店だけピン表示」** → 50店は記録ゼロなので座標入れても出ない → **map 改修は必須**（条件付きでなく確定）。
...
- **UI 方向 確定**:
  ...
  - **フライホイール要件を UI で具体化**: 会食記録に ★＋良かった点＋反省・申し送り＋業務成果。反省は該当店の減点に反映（例「騒がしい」→次回減点）。店舗詳細に4軸スコア（格式/予算/シーン/特別感）＋競合ドリンク確認（確信度付き）。
...
- **UI ⇄ 要件のギャップ分析 → B案で確定**（UI=ビジョン／v1=薄く）。spec/design をブラッシュアップ（commit `317be55`）:
  ...
  - ゲスト管理・お気に入り・ダッシュボード・会食予定・学習は【拡大】（UI に存在するが v1 では作らない）。
...
- **店DBの型と京橋 seed を in-repo で実体化**（`data/`）:
  ...
  - `data/venues.seed.json` — **京橋・八重洲・日本橋の会食店 30件**（research agent(sonnet) が公式サイト裏取り。検証済み・エラーなし。格式S5/A18/B7・予算7,800〜36,300円・個室あり27/30・ジャンル12種）
